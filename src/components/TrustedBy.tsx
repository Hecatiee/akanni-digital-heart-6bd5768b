            ...
            <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-6 sm:gap-x-12 md:gap-x-16 lg:gap-x-24 gap-y-6">
              {topRow.map((company, index) => (
                <span
                  ...
                  className={`inline-block text-2xl sm:text-3xl md:text-4xl lg:text-5xl text-[hsl(0,0%,35%)] hover:text-[hsl(0,0%,20%)] transition-all duration-300 hover:-translate-y-2 hover:scale-110 animate-float ${company.style}`}
                >
                  {company.name}
                </span>
              ))}
